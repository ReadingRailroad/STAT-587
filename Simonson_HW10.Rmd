...
- __Answer:__ The multiple $R^2$ from our model in _B)_ 0.9348, which means that 93.48% of the overall variation in bear weights is explained by our model. 
...
pred1<-predict(mod,newdata = bearrr,se.fit=T)
pred1
```
...
y.hat<-198.1687
se.hat<-9.904018
t.star<-2.021 # from t-table with 40 degrees of freedom

lower<-y.hat-(t.star*se.hat)
upper<-y.hat+(t.star*se.hat)
...
- __Answer:__ The normality assumption is questionable because of curvature in the qq-plot. 
...
- __Answer:__ One would need 7 dummy variables. The reference level would be April
...
```{r}
mod<-lm(Eggs~Genotype + Weight, data = mpi)
summary(mod)
```
