...
In a study to test the efficacy of a diet in reducing obesity, 20 obese participants (initial BMI greater than or equal to 30) were enrolled. One participant dropped out after few weeks. The participants’ BMI values were observed before the program started and after following the diet plan for one month. The data in _bmi.csv_ givs the change in BMI of these 19 participants. Suppose $\mu$ denotes the average change in BMI due to the diet. The null hypothesis is then $H_0: \mu = 0$.
...
- __Answer:__ The p-value for the two sided t-test of mean changes in BMI are $\mu \neq 0$ is 0.0002962, which provides very strong evidence that the true difference in BMI is not equal to zero.

(d) Between one-sided and two-sided alternatives which one do you think is more objective and why?

- __Answer:__ I believe that the two-sided test is more objective because it accounts for the fact that there may be extreme values in either direction of the null hypothesis $H_0: \mu = 0$ instead of an only greater than or less than 0. In this example, the data suggest that the true difference in BMI is not equal to zero, but if the person applying the test had completed a one-sided test to determine if the average change in BMI was less than zero, they would fail to reject the null hypothesis.
...
(d) Is is appropriate to make a causal conclusion from this study? Briefly explain.

- __Answer:__ The assumption of independent observation of randomly assigned treatments cannot be tested here, since the success of gene injection is nonrandom. Further, the p-value greater than 0.1 does not provide evidence to reject the null hypothesis that the mean weights are equal between groups.
...
- __Answer:__ The "Oatmeal Haters" group has more variation because of a larger standard devation. Given that there is a greater number of "Oatmeal Haters" the precision of that estimate should be higher than the precision of the "Oatmeal Eaters" group.
...
```
- __Answer:__ We are 99% confident that the difference in mean blood pressure values is 9.703327 to 30.29667 units __(blood pressure units not provided)__, which indicates that "Oatmeal Haters" (in this study only) have a higher average blood pressure compared to "Oatmeal Eaters" (in this study only).
...
beak.diff<- mean(dat$Depth[dat$Year == "1976"]) -
            mean(dat$Depth[dat$Year == "1978"])
beak.diff
...
- __Answer:__ The 95% confidence interval, already provided by R output in 3(b), and re-calculated in 3(c), is -0.7626008 to -0.3122058.
...
- __Answer:__ See data frame "Five.A" immediately below.
```{r}
Five.A
```
...
- Let $\mu_1$ be the mean duration of visit for Queen Bees, and $\mu_2$ be the mean duration of visit for Worker Bees. Therefore:
...
- __Answer:__ The t-ratio from the pooled two-sided t-test is 4.0787 with 45 degrees of freedom and a p-value of 0.0001824. This provides strong evidence that there is a difference between worker and queen bees in the duration of time spent at a flower.
...
- __Answer:__ From 5(b), we can see that the 90% confidence interval is 11.56609 to 27.75772. Therefore, we are 90% confident that the true difference of mean length of visit lies between 11.56609 and 27.75772 units of time, probably seconds. 
...
- __Answer:__ These plots show two things: we have unequal variance, which is inappropriate for a reguar pooled t-test, and that deviation from the QQ line indicates that there may be some non-normal data in the "Q" group and at high values of the "W" group. Because an inappropriate test was used, the results are an output of a futile exercise that did not make worthwhile conclusions. Perhaps the researcher should have tested the assumptions of the t-test _before_ running the t-test.