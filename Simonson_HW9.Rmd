...
- __Answer:__ Let's start with the F-table as we know it (Let Y = :

Source  | DF   | SS              | MS              | F           | p-value
--------|------|-----------------|-----------------|-------------|---------------
Model   | m    | $SS_{model} = \sum_{i=1}^n (\hat{y_i} - \bar{y})^2$ | .               | $F = \frac{MS_{model}}{\hat{\sigma}^2}$            | .
Error   | n-m  | $SS_{error} = \sum_{i=1}^n (y_i - \bar{y})^2$       | .               | .           | . 
Total   | n-1  | $SS_{total} = \sum_{}               | .               | .           | .
...
- __Answer:__ The predicted median abundance of longnose dace for 75 km of stream with these characteristics is 111 (rounded to whole number for individual fish).
...
- __Answer:__ We would expect the median brain weight to be 10.10. We are 95% confident that the median brain weight for red kangaroos will be between 2.38 and 42.89. 
